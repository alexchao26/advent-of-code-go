...
func returnCodes(number int) []int {
	ans := make([]int, 4)
	ans[0] = number % 100
	ans[1] = (number % 1000) / 100
	ans[2] = (number % 10000) / 1000
	ans[3] = (number % 100000) / 10000
...
func runDiagnostics(puzzleInput []int, inputValue int) string {
	...
	for i := 0; i < len(puzzleInput); {
		...
		param1 := slicedInstruction[1]
		param2 := slicedInstruction[2]
		...
		if opCode == 99 {
			...
			// one more input, param1 is position or immediate mode
			...
			// return a value opCode, in immediate mode?
			// always position mode
			indexPlusOne := puzzleInput[i+1]
			fmt.Println("output for position: ", puzzleInput[indexPlusOne])
			i += 2
		} else {
